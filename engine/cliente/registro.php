        ...
        if(isset($_POST)){
            ...
            foreach($_POST AS $key => $value){ 
                ...
                if($distinto && (!isset($value) || is_null($value) || $value == '')){
                    Sistema::debug('Alert', 'engine > cliente > registro.php - Hubo un error con el valor del input ['.$key.'].');
            ...
            Cliente::registro($data);
        }else{
            $mensaje['tipo'] = 'danger';
            $mensaje['cuerpo'] = 'Hubo un error al recibir la información. <b>Intente nuevamente o contacte al administrador.</b>';
            Alert::mensaje($mensaje);
            Sistema::debug('Error', 'engine > cliente > registro.php - Hubo un error al recibir la información del formulario.');
        }
    }else{
        Sistema::debug('Error', 'engine > cliente > registro.php - Usuario no logueado.');
    }
?>