        ...
        if(isset($_POST)){
            ...
            foreach($_POST AS $key => $value){ 
                ...
                if($distinto && (!isset($value) || is_null($value) || $value == '')){
                    Sistema::debug('Alert', 'engine > compania > stock-editar-contenido.php - Hubo un error con el valor del input ['.$key.'].');
            ...
            if($data["tipo"] === $data["tipo2"] && $data["idProducto"] === $data["idProducto2"]){
                Compania::stockEditarContenido($data);
            }else{
                Sistema::debug('Alert', 'engine > compania > stock-editar-contenido.php - Hubo un error con los valores comparados de tipo y producto.');
            ...
            Sistema::debug('Error', 'engine > compania > stock-editar-contenido.php - Hubo un error al recibir la información del formulario.');
        }
    }else{
